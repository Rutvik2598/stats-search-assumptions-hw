...
class WelfordNum(Num):
  ...
  def mid(self):
    return self.mu  # TODO: return self.mu

  def spread(self):
    return math.sqrt(self.m2/(self.n-1)) if self.n > 1 else 0  # TODO: sqrt(m2/(n-1)) if n>1 else 0
  ...
  def pick(self, v=None):
    ...
    base = self.mu if v is None or v == "?" else v
    result = random.gauss(base, self.spread())
    return max(self._lo(), min(self._hi(), result))
